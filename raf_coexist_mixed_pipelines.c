...
// Critério de "nó difícil"
#define HARD_ITERS_THRESH 30
#define HARD_FX_THRESH    1e-6
...
typedef struct HyperNode {
    ...
    double    x0;             // chute inicial usado na gênese
...
static void build_problem_for_node(HyperNode *n, int i) {
    ...
    if (t >= TYPE_POLY2 && t <= TYPE_POLY6) {
        ...
        double mag = fabs(res.root);
        if (mag > 1e6) mag = 1e6;
        long long scaled = (long long) llround(mag * 1e6);
        if (scaled < 0) scaled = 0;
        n->solution_cache = (uint64_t) scaled;
    }
...
// ===================== CRITÉRIO DE NÓ DIFÍCIL =====================
static int is_hard_node(const HyperNode *n) {
    if (!n->converged) return 1;
    if (n->newton_iters >= HARD_ITERS_THRESH) return 1;
    if (n->final_fx > HARD_FX_THRESH) return 1;
    return 0;
}
...
static void init_pipe_stats(PipeStats *ps) {
    ps->count = 0;
    ps->iters = 0;
    ps->converged = 0;
    ps->min_iters = 1000000000;
    ps->max_iters = 0;
    ps->sum_fx = 0.0;
}

// Pipeline A: reprocessar os mesmos problemas difíceis
static void run_pipeline_same(const HyperNode *nodes, const int *hard_idx, int hard_count, PipeStats *ps) {
    init_pipe_stats(ps);

    for (int i = 0; i < hard_count; i++) {
        int idx = hard_idx[i];
        const HyperNode *orig = &nodes[idx];

        NewtonResult res = newton_solve(orig, orig->x0);

        ps->count++;
        ps->iters += res.iters;
        ps->sum_fx += res.final_fx;
        if (res.converged) ps->converged++;

        if (ps->min_iters > res.iters) ps->min_iters = res.iters;
        if (ps->max_iters < res.iters) ps->max_iters = res.iters;
    }
}

// Pipeline B: sortear novas variáveis (novos coeficientes/chutes) mantendo o tipo
static void run_pipeline_random(const HyperNode *nodes, const int *hard_idx, int hard_count, PipeStats *ps) {
    init_pipe_stats(ps);
    ...
    for (int i = 0; i < hard_count; i++) {
        ...
        tmp.problem_type = orig->problem_type;
        tmp.degree       = orig->degree;

        uint64_t rng = aetl_seed_from_id(idx ^ 0xA5B3571U);

        if (tmp.problem_type >= TYPE_POLY2 && tmp.problem_type <= TYPE_POLY6) {
            int degree = tmp.degree;
            tmp.coeff[degree] = 1.0;
            for (int k = 0; k < degree; k++) {
                int coef_i = aetl_rand_int(&rng, -3, 3);
                tmp.coeff[k] = (double) coef_i;
            }
            double x0 = aetl_rand_uniform(&rng, -5.0, 5.0);
            NewtonResult res = newton_solve(&tmp, x0);
...
int main(int argc, char **argv) {
    int depth = 1000000; // 100x mais nós (padrão)
    if (argc > 1) {
        int tmp = atoi(argv[1]);
        if (tmp > 0) depth = tmp;
    }

    printf("=== RAFAELIA COEXISTENCE ENGINE (MIXED + PIPELINES) ===\n");
    printf("Meta: universo misto 2..6 + trig, com ramificação em 2 pipelines sobre nós difíceis.\n\n");
    ...
    printf("\n📊 Estatísticas de Newton–Raphson (GÊNESE)\n");
    ...
    for (int t = TYPE_POLY2; t <= TYPE_TRIG3; t++) {
        ...
        printf("     %-24s : nós=%8lld | iters_tot=%10lld | it_med=%.3f | conv=%.2f%%\n",
               type_name(t), c, it, avg_it, cr);
    }

    // ===================== COLETA DOS NÓS DIFÍCEIS =====================
    int *hard_idx = (int *) malloc(depth * sizeof(int));
    if (!hard_idx) {
        fprintf(stderr, "[ERRO] malloc falhou para hard_idx.\n");
        free(universe);
        return 1;
    }
    ...
    int hard_count = 0;
    for (int i = 0; i < depth; i++) {
        if (is_hard_node(&universe[i])) {
            hard_idx[hard_count++] = i;
        }
    }

    printf("\n🔍 Nós difíceis identificados: %d (critério: !converged || iters>=%d || |f|>%g)\n",
           hard_count, HARD_ITERS_THRESH, HARD_FX_THRESH);

    // ===================== PIPELINE A: MESMOS PROBLEMAS =====================
    PipeStats ps_same;
    run_pipeline_same(universe, hard_idx, hard_count, &ps_same);

    double avg_it_same = (ps_same.count > 0) ? (double) ps_same.iters / (double) ps_same.count : 0.0;
    double conv_rate_same = (ps_same.count > 0) ? (double) ps_same.converged * 100.0 / (double) ps_same.count : 0.0;
    double avg_fx_same  = (ps_same.count > 0) ? ps_same.sum_fx / (double) ps_same.count : 0.0;

    printf("\n🧬 PIPELINE A (MESMOS PROBLEMAS)\n");
    printf("   Nós difíceis reprocessados : %lld\n", ps_same.count);
    printf("   Iterações totais           : %lld\n", ps_same.iters);
    printf("   Iterações mín / máx        : %d / %d\n", ps_same.min_iters, ps_same.max_iters);
    printf("   Iterações médias/nó        : %.3f\n", avg_it_same);
    printf("   Convergências              : %lld (%.2f%%)\n", ps_same.converged, conv_rate_same);
    printf("   |f(x_final)| médio         : %.6e\n", avg_fx_same);

    // ===================== PIPELINE B: VARIÁVEIS SORTEADAS =====================
    PipeStats ps_rand;
    run_pipeline_random(universe, hard_idx, hard_count, &ps_rand);

    double avg_it_rand = (ps_rand.count > 0) ? (double) ps_rand.iters / (double) ps_rand.count : 0.0;
    double conv_rate_rand = (ps_rand.count > 0) ? (double) ps_rand.converged * 100.0 / (double) ps_rand.count : 0.0;
    double avg_fx_rand  = (ps_rand.count > 0) ? ps_rand.sum_fx / (double) ps_rand.count : 0.0;

    printf("\n🎲 PIPELINE B (VARIÁVEIS SORTEADAS)\n");
    printf("   Nós difíceis reprocessados : %lld\n", ps_rand.count);
    printf("   Iterações totais           : %lld\n", ps_rand.iters);
    printf("   Iterações mín / máx        : %d / %d\n", ps_rand.min_iters, ps_rand.max_iters);
    printf("   Iterações médias/nó        : %.3f\n", avg_it_rand);
    printf("   Convergências              : %lld (%.2f%%)\n", ps_rand.converged, conv_rate_rand);
    printf("   |f(x_final)| médio         : %.6e\n", avg_fx_rand);
