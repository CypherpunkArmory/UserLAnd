...
class FilesystemDaoTest {
    ...
    @Test
    fun returnsEmptyListIfFilesystemDoesNotExist() {
        val fs = db.filesystemDao().getFilesystemByName(DEFAULT_NAME)
    ...
    fun deleteFilesystemAndFailRetrieval() {
        ...
        val id = db.filesystemDao().getFilesystemByName(DEFAULT_NAME).first().id
    ...
    @Test
    fun test() {
        val fs = db.filesystemDao().getFilesystemByName("apps")
        val t = if (fs.isEmpty()) "1" else "2"
        assertTrue(t == "1")
    }
