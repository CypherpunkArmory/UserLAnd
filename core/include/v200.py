...
class RafaeliaTrinity:
            ...
            if ext in ['.json', '.jsonl']:
                for data, origin in MassiveIngestor.stream_json(proc_path):
                    ...
                    self.hippocampus.consolidate(Engram(
                        ...
                        type="knowledge_node",
                        vector=list(vec),
                        payload={"src": proc_path.name, "keys": list(data.keys())}
                    ))
                    items_count += 1
                    if items_count % 2000 == 0: logger.info(f"... {items_count} itens absorvidos")
    ...
    def interact(self, user_query: str):
        ...
        print(f"   Ressonância Secundária: {memories[1][0].type} ({memories[1][1]:.4f})" if len(memories)>1 else "")
        print()
