...
void _0() { /* Genesis */
    ...
    float *p = X.M;
    for(int i=0; i<N*D; i++) *p++ = ((float)(_r(&X.S)%2000)/1000.0f) - 1.0f;
}
...
void _1() { /* Hyper-Process (n9 Evolution) */
    ...
    while(p < e) {
        ...
        /* Update Octonion part (0-7) */
        for(int k=0; k<8; k++) {
            /* Diffusão não-linear baseada no estado phi */
            p[k] = (p[k] * 0.99f) + (tmp[k] * 0.05f * phi);
            norm += p[k]*p[k];
        }
        
        /* 3. Update Phi (Observer / Energy) */
        /* n9 regula a energia do sistema para evitar explosão */
        norm = sqrtf(norm);
        p[8] = _sig(phi + (norm - 1.0f) * 0.1f);
...
void _2() {
    ...
    printf("\r[CYC %lu] S:%08X | NRG:%.2f | ZETA(n9):%.4f | IOPS: ULTRA", 
           X.C, X.S, E/N, Z/N);
    fflush(stdout);
}
...
int main() {
    printf("\033[2J\033[H");
    printf("\x1b[36m=== RAFAELIA OCTONION CORE [v9.0] ===\x1b[0m\n");
    printf("DIM: %d (8 Oct + 1 Phi) | NODES: %d | MEM: %lu KB\n", 
           D, N, (unsigned long)(sizeof(_X)/1024));
    
    _0(); /* Init */
    
    /* Loop Atemporal (Executa por ciclos definidos ou infinito) */
    clock_t t0 = clock();
    
    /* Rodar 10000 épocas de aquecimento */
    ...
    printf("\n\n\x1b[32m[DONE]\x1b[0m\n");
    /* Cálculo de Giga-Ops (8 muls + 8 adds + 8 adds no cross product... aprox 200 ops per node) */
